...
$( '#add-pop-item' ).on('click', function(){
    ...
    if(text){
        ...
        alert("Please provide a message to create the message file!", "Message", 48)
    }
})
...
function initList(){
    ...
    <div class="pop-message-item">
        ...
        <div class="icon-item-wrap">
            <div class="icon-item icon_${post.icon}"></div>
        </div>
        <div class="item-content">
            <h2 class="message-item-title">${post.title}</h2>
...
function createPopUp(){
    ...
    const code = Object.values(pop_items).map(post => `
        X=Msgbox("${post.text}", ${post.icon} , "${post.title}")
    `)

    download('lol.vbs', code.join(''))
